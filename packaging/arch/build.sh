...
Examples:
  ...
  packaging/arch/build.sh --work-dir /var/tmp/songonsole
EOF
}
...
work="$(package_work_dir songonsole-arch)"
cleanup() {
    if [[ -n "${work:-}" && "$work" == */songonsole-arch.* && -d "$work" ]]; then
        rm -rf -- "$work"
    fi
}
trap cleanup EXIT
...
source_archive="$work/songonsole-$PACKAGE_VERSION.tar.gz"
