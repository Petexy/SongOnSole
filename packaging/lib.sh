...
# manifest, the spec, the AppStream release list and `songonsole --version`
...
PACKAGE_WORK_ROOT="${SONGONSOLE_WORK_DIR:-$PACKAGING_DIR/out/build}"
...
# A relative SONGONSOLE_WORK_DIR has to become absolute here rather than at
...
if [[ -n "${SONGONSOLE_WORK_DIR:-}" ]]; then
    package_set_work_root "$SONGONSOLE_WORK_DIR"
fi
...
Point the build somewhere with room using --work-dir DIR (or SONGONSOLE_WORK_DIR),
