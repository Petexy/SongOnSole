...
profile. SONGONSOLE_WORK_DIR sets the same thing.
USAGE
}
...
work="$(package_work_dir songonsole-fedora)"
cleanup() {
    if [[ -n "${work:-}" && "$work" == */songonsole-fedora.* && -d "$work" ]]; then
        rm -rf -- "$work"
    fi
}
trap cleanup EXIT
...
archive_snapshot "$source_dir" "$work/SOURCES/songonsole-$PACKAGE_VERSION.tar.gz"
install -m0644 "$script_dir/songonsole.spec" "$work/SPECS/songonsole.spec"
