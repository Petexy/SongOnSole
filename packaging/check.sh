...
# `songonsole --version` prints CARGO_PKG_VERSION, so the manifest is where a
...
[[ "$locked_version" == "$PACKAGE_VERSION" ]] \
    || package_die "Cargo.lock still says songonsole $locked_version, not $PACKAGE_VERSION.
Run: scripts/bump-version.sh $PACKAGE_VERSION"
...
grep -Eq "^Version:[[:space:]]+${PACKAGE_VERSION//./\\.}\$" \
    "$PACKAGING_DIR/fedora/songonsole.spec" \
    || package_die "fedora/songonsole.spec does not declare version $PACKAGE_VERSION"
...
newest_release="$(awk '
    match($0, /<release version="[^"]+"/) {
        ...
        print line
        exit
    }' "$PROJECT_ROOT/data/io.github.petexy.songonsole.metainfo.xml")"
[[ "$newest_release" == "$PACKAGE_VERSION" ]] \
    || package_die "the metainfo's newest release is $newest_release, not $PACKAGE_VERSION.
Add a <release> for $PACKAGE_VERSION to data/io.github.petexy.songonsole.metainfo.xml"
...
grep -Eq "^BuildRequires:[[:space:]]+lxb-toolkit-devel >= ${toolkit_version//./\\.}\$" \
    "$PACKAGING_DIR/fedora/songonsole.spec" \
    || package_die "fedora/songonsole.spec does not build-require lxb-toolkit-devel >= $toolkit_version.
...
desktop_name="$(awk -F= '/^Name=/ { print substr($0, 6); exit }' \
    "$PROJECT_ROOT/data/songonsole.desktop")"
...
# The window is `lxb-app`'s, so the name it opens under is the second argument
# to `App::new` — the first being the stable id, which is the fourth name and is
# checked below. Since it was translated that name is a *message* rather than a
# constant, so the id is read out of the source and the word out of `en-GB.ftl`
# — the fallback catalogue, which is the language the desktop entry's own
# untranslated `Name=` is in. Comparing against any other catalogue would be
# comparing two languages and failing every time.
window_title_id="$(grep -oE 'App::new\(APP_ID, i18n::text\("[^"]*"\)\)' \
...
program="$target_dir/release/songonsole"
[[ -x "$program" ]] || package_die "missing release binary: $program
Run without --no-build, or build it first."
reported="$("$program" --version)"
[[ "$reported" == "songonsole $PACKAGE_VERSION" ]] \
    || package_die "the binary reports '$reported', not 'songonsole $PACKAGE_VERSION'"

package_note "staging the payload"
stage="$(package_work_dir songonsole-check)"
cleanup() {
    if [[ -n "${stage:-}" && "$stage" == */songonsole-check.* && -d "$stage" ]]; then
        rm -rf -- "$stage"
    fi
}
trap cleanup EXIT
...
for expected in \
    usr/bin/songonsole \
    usr/share/applications/songonsole.desktop \
    usr/share/icons/hicolor/scalable/apps/songonsole.svg \
    usr/share/metainfo/io.github.petexy.songonsole.metainfo.xml; do
...
exec_name="$(awk -F= '/^Exec=/ { print $2; exit }' \
    "$stage/usr/share/applications/songonsole.desktop" | awk '{print $1}')"
icon_name="$(awk -F= '/^Icon=/ { print $2; exit }' \
    "$stage/usr/share/applications/songonsole.desktop")"
...
if command -v desktop-file-validate >/dev/null 2>&1; then
    desktop-file-validate "$stage/usr/share/applications/songonsole.desktop" \
...
if command -v appstreamcli >/dev/null 2>&1; then
    ...
    appstreamcli validate --no-net --pedantic \
        "$stage/usr/share/metainfo/io.github.petexy.songonsole.metainfo.xml" \
...
package_note "all checks passed for songonsole $PACKAGE_VERSION"
