...
//! writing, and a song called `{ $name }` is still a song. That is why every
//! error in `audio.rs` and `library.rs` is a message with the path or the
//! trouble in it rather than a sentence built up out of pieces.
use std::sync::LazyLock;

pub use lxb_toolkit::i18n::{Catalog, FluentArgs};

/// Every language this application is translated into. A new one is a file
/// beside `en-GB.ftl` and a line here; one that is missing a message falls
...
///
/// A count is passed as a number — `"count" => songs` — never as text: a
/// language picks the form of the noun by looking at the number, and Fluent
/// handed a string falls silently to the form nothing else uses. French counts
/// nought singular and Spanish does not, and neither rule is written anywhere
/// in this repository; both arrive only because the number is a number.
#[macro_export]
macro_rules! message {
...
mod tests {
    ...
    /// them, and every one of them formats. An overlay is held to the opposite
    /// rule: everything in it must be a message English has, take the same
    /// variables, and actually say something else.
    #[test]
    ...
    /// strings rather than glyphs would say a word about it. The Han face is a
    /// subset of GB 2312, so this also says when a sentence has to be reworded
    /// rather than the face regrown.
    #[test]
