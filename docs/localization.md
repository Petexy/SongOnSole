...
Music speaks the ten languages LineXinBar does: **German**, **English (UK)**,
...
To see a page in another language without changing anything:

```sh
LC_ALL=pl_PL.UTF-8 songonsole
LC_ALL=zh_CN.UTF-8 songonsole --demo
```
...
* `locales/en-US.ftl` — an **overlay** of what America writes differently,
  which here is three messages, all of them the one word *favourites*.
  Everything not in it is answered out of `en-GB.ftl`, and `Catalog::validate`
  fails the file if it ever copies a message across unchanged.
...
A song's title, its artist, the album it came from and the name of a folder
somebody added are never translated, and neither is anything another program
said when it failed: `ffprobe` and the sound library speak for themselves, and
their words arrive as `{ $error }` inside a sentence this application wrote.

The `--help` text is English and stays English. It is read in a terminal, which
is where the rest of this machine answers in English too.

## Two traps this application walks into and out of

**A count is passed as a number.** `message!("song-count", "count" => songs)`
hands Fluent an integer, and Fluent asks CLDR which form of the noun that
number takes. Handed a *string* it matches no variant at all and every language
silently falls to `other` — which is right in English and wrong in French at
exactly one value, nought.
...
stands beside it. `draw::fit` is what every label goes through first, and it
asks the measurer rather than counting letters: a Chinese character is a whole
em where a Latin one is about two thirds of one, so a count of letters is a
guess that is wrong in one language out of two.
...
`lxb_toolkit::i18n::LANGUAGES`. Translate `data/songonsole.desktop`
(`Name[xx]`, `GenericName[xx]`, `Comment[xx]`, `Keywords[xx]`) and the
AppStream metadata. Keep every identifier and variable name as English has
them, put the variables in the order the language wants, and give it as many
plural forms as it has.
...
these catalogs through the shipped faces alone and fails on the first character
none of them has — which is also what says a Chinese sentence has to be
reworded, since the Han face is cut to GB 2312.
...
Three of those tests are about the catalogs: that every language carries the
same messages with the same variables and that each of them formats, that every
identifier the sources ask for is one the catalogs have, and that every word
they carry can be drawn by a face the toolkit ships. A test that compares a
label against English text is a test that fails on a Polish machine, so where a
test needs the words it asks the catalog for them the way the code does.
