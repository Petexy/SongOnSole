...
# Cargo's manifest is one: `songonsole --version` prints CARGO_PKG_VERSION, so
...
spec="$project_root/packaging/fedora/songonsole.spec"
metainfo="$project_root/data/io.github.petexy.songonsole.metainfo.xml"
...
packaging/fedora/songonsole.spec and the AppStream release list. Every other
...
note "packaging/fedora/songonsole.spec Version: -> $new_version"
awk -v version="$new_version" '
    /^Version:[[:space:]]/ && !done {
        printf "Version:        %s\n", version
        done = 1
...
if grep -Fq "<release version=\"$new_version\"" "$metainfo"; then
    ...
    note "data/io.github.petexy.songonsole.metainfo.xml <- $new_version"
    today="$(date -u +%Y-%m-%d)"
...
echo "    packaging/fedora/songonsole.spec   %changelog"
echo "    data/io.github.petexy.songonsole.metainfo.xml   what this release changed"
